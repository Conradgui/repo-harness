...
**分支测试是红的**，其中一部分是该分支自己引入的——`git show 2d62a8c` 能看到它引入的 3 个 `ConfigError: state_root_unavailable`。

> 具体失败**数量**不在此断言：suite 计数依赖运行环境，理由见交付报告 3.1。
...
**A · 在 codex 分支上解锁**
需要：恢复被删的实现、拆四层封锁、修掉那些失败测试、拆掉一个「代码不许变短」的测试、处理 state store 回归。**实质等于把 main 已有的代码重写一遍**，同时继续背着 6,374 行文档沉积。
