...
# Branch prefix auto-generated by Auto Issue Fix runs. Exempted from the
# maintainer trust gate so that the tool's own naming convention does not
# block otherwise clean PRs. If the branch naming scheme changes, update
# this pattern — the gate relies on it to distinguish tool-generated branches
# from user-supplied ones that should be scrutinised.
AUTO_ISSUE_FIX_BRANCH_RE = re.compile(r"repo-harness-auto-issue-fix-\d+")
...
def maintainer_trust_block_reason(*, title: str, body: str, commit_message: str, branch: str) -> str:
    ...
    for field_name, value in fields.items():
        if field_name == "branch" and AUTO_ISSUE_FIX_BRANCH_RE.fullmatch(str(value or "")):
            continue
