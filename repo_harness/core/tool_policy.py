...
class ToolPolicy:
    ...
    def decision(self, name, args):
        ...
        if name == "run_shell" and SHELL_SEARCH_RE.search(str(args.get("command", "")).strip()):
            command = str(args.get("command", "")).strip()
            reason = "shell_search_should_use_tool" if re.search(r";|&&|\|\|", command) else "tool_policy_workspace_read"
            return ToolPolicyDecision.deny(
                reason,
                "error: tool policy rejected run_shell workspace read/search; use search/read_file/list_files first",
            )
    ...
    def record_result(self, name, args, metadata):
        ...
        elif name in {"write_file", "patch_file"} and rel:
            self._self_authored.add(rel)
            self._fresh_reads.discard(rel)
        elif name == "run_shell" and metadata.get("workspace_changed"):
    ...
    def _has_fresh_read(self, path, *, allow_self_authored=False):
        ...
        if not rel:
            return False
        if rel in self._fresh_reads or (allow_self_authored and rel in self._self_authored):
            return True
        try:
            summary = self.agent.memory.to_dict().get("file_summaries", {}).get(rel, {})
            return bool(summary and summary.get("freshness") == memorylib.file_freshness(rel, self.agent.root))
        except Exception:
            return False

    @staticmethod
    def _prior_read_required(tool_name, path):
        return ToolPolicyDecision.deny("prior_read_required", f"error: {tool_name} requires a fresh read_file of {path} before modifying it")
