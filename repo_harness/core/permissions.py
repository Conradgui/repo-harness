...
class PermissionChecker:
    ...
    def check(self, tool_or_name, args=None):
        ...
        if profile is not None and not profile.allows(tool.name):
            if profile_name == "plan":
                return PermissionDecision.deny("plan_mode_tool_not_allowed", "plan_mode_write_guard", profile=profile_name)
    ...
    def _check_plan(self, tool, args, profile_name):
        ...
        if Path(requested) != Path(active):
            return PermissionDecision.deny("plan_mode_path_mismatch", "plan_mode_write_guard", profile=profile_name)
