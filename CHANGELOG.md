...
**版本说明**：仓库内 v1–v7 是开发迭代代号（内部里程碑），不是发布版本。项目尚未对外发布正式 release，首个公开版本从 `0.1.0` 起步；每次正式发布前，`[Unreleased]` 的内容会归入对应版本段。

## [Unreleased] — 0.1.0（首个对外发布，待定）

### 本轮未发布条目（OSS 成熟度加固进行中）

- 文档体系：新增根目录 `CONTRIBUTING.md`、`SECURITY.md`、`CODE_OF_CONDUCT.md`、`RELEASING.md`，CHANGELOG 从草稿提升到根目录。
- 工程配置：`pyproject.toml` 补全 `classifiers` / `keywords` / `[project.urls]` 元数据；新增 `.github/` issue 与 PR 模板；新增 `py.typed`。
- CI：测试命令增加覆盖率门禁（阈值以实测为准）。
- 代码健康：`cli.py` 命令分发注册表化（纯重构，零行为变更）；引入 `logging` 诊断通道。

### 发布前必须完成

- 确认 changelog 条目与代码一致（ADR-006：文档数字必须可由命令复算）。
- 全量门禁：`uv run ruff check .` 0 error；`uv run pytest tests/ -q` 全绿；文档一致性测试全绿。

---

## v7 — 2026-07：Builder 提取收尾、Sandbox 加固与测试质量门禁
...
- `uv run pytest tests/ -q` — 559 passed, 1 skipped（2026-08-06 全量复核；此前 520 passed 记录已更新，G1/G2/G3 新增测试后全量 560 用例）
- `uv run ruff check .` — 0 error

---

## v6 — 2026-07：深度审计、God Object 解体推进与安全加固
...
## v5 — 2026-06：动态上下文预算、死代码清理与代码质量提升
...
## v4 — 2026-05：代码清理、安全加固与 Claude Code Skill 兼容
...
## v3 — 2026-04：能力完善与 Auto Issue Fix v2 真实执行
...
v2（Pico 时期的能力整合）与 v1（初始骨架）的开发历史保留在 `docs/maintainer-prep/changelog-draft.md`。从 v3 起，变更记录统一以本文件为准。
